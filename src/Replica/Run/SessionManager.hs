...
import Control.Concurrent.STM (STM, TVar, atomically, modifyTVar', newTVar, readTVar, retry, throwSTM, writeTVar)
import Control.Exception (bracket, mask, mask_, onException)
...
data SessionManage = SessionManage
    ...
    , -- | 初期接続待ち
      smOrphans0 :: TVar (PSQ.OrdPSQ SessionID Ch.Time Session)
    , -- | 再接続待ち
      smOrphans :: TVar (PSQ.OrdPSQ SessionID Ch.Time Session)
    }
